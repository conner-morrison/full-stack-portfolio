...
export default function Hero() {
  ...
  const cardPadding = "clamp(1.25rem, 5vw, 2.5rem)";
  const sectionPadding = "clamp(1rem, 4vw, 2rem)";

  return (
    <Parallax speed={30}>
      <section
        className="relative w-full min-w-0 overflow-hidden rounded-b-2xl sm:rounded-b-3xl flex items-center justify-center"
        style={{
          minHeight: "min(50vh, 50dvh)",
          padding: sectionPadding,
        }}
      >
        {/* Rounded card container: wraps content, no fixed height */}
        <div
          className="relative w-full max-w-[min(48rem,92vw)] rounded-[clamp(1.25rem,4vw,2.5rem)] overflow-hidden border border-white/40 dark:border-white/8 shadow-[0_26px_90px_-42px_rgba(15,23,42,0.26)] dark:shadow-[0_26px_90px_-42px_rgba(0,0,0,0.6)] ring-1 ring-black/5 dark:ring-white/5 flex flex-col items-center justify-center min-h-0"
          style={{
            padding: cardPadding,
            minHeight: "min(20rem, 50dvh)",
          }}
        >
          {/* Background layers: fill card, behind content */}
          <div className="absolute inset-0 -z-10" aria-hidden>
            <div
              className="absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.78)_0%,rgba(242,245,249,0.72)_42%,rgba(230,233,238,0.64)_100%)] dark:bg-[linear-gradient(180deg,rgba(5,10,20,0.96)_0%,rgba(8,14,26,0.94)_42%,rgba(6,12,22,0.96)_100%)]"
              aria-hidden
            />
            <div
              className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_18%,rgba(255,255,255,0.68)_0%,rgba(255,255,255,0.24)_28%,transparent_58%),radial-gradient(circle_at_18%_78%,rgba(59,130,246,0.12)_0%,transparent_26%),radial-gradient(circle_at_84%_24%,rgba(14,165,233,0.1)_0%,transparent_22%)] dark:bg-[radial-gradient(ellipse_at_50%_18%,rgba(255,255,255,0.02)_0%,transparent_28%,transparent_58%),radial-gradient(circle_at_18%_78%,rgba(59,130,246,0.08)_0%,transparent_26%),radial-gradient(circle_at_84%_24%,rgba(14,165,233,0.06)_0%,transparent_22%)]"
              aria-hidden
            />
          ...
          {/* Content: flex column, responsive typography */}
          <div className="relative z-0 w-full flex flex-col items-center justify-center text-center min-w-0">
            <motion.div
              variants={container}
              initial="hidden"
              animate="visible"
              className="flex flex-col items-center"
              style={{ gap: "clamp(0.75rem, 2.5vw, 1.25rem)" }}
            >
              <motion.h1
                variants={line}
                className="font-serif font-semibold tracking-tight text-[var(--foreground)] w-full"
                style={{ fontSize: "clamp(2rem, 6vw + 1rem, 3.75rem)" }}
              >
                Conner Morrison
              </motion.h1>
              ...
              <motion.p
                variants={line}
                className="text-[var(--muted)] font-medium w-full"
                style={{ fontSize: "clamp(0.9375rem, 2vw + 0.5rem, 1.25rem)" }}
              >
              ...
              <motion.p
                variants={lineWipe}
                className="text-[var(--muted)] leading-relaxed max-w-xl mx-auto w-full min-h-[2.75rem] flex flex-wrap justify-center items-center"
                style={{ fontSize: "clamp(0.875rem, 1.5vw + 0.5rem, 1.125rem)" }}
              >
              ...
              >
                <a
                  ...
                  download
                  className="inline-flex items-center justify-center min-w-[120px] sm:min-w-[140px] px-4 sm:px-6 py-3 sm:py-3.5 rounded-xl bg-[var(--accent)] text-white font-semibold shadow-lg shadow-[var(--accent)]/25 hover:opacity-90 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
                  style={{ fontSize: "clamp(0.8125rem, 1.5vw + 0.25rem, 0.875rem)" }}
                >
                  Download Resume
                </a>
              </motion.div>
